...
export default function LandingPage() {
    ...
    return (
        <div className="responsive-page min-h-screen bg-[var(--bg-main)] flex flex-col font-sans transition-colors duration-300">
            {/* Header */}
            <header className="px-4 sm:px-6 py-4 sm:py-6 flex justify-between items-center border-b-2 border-[var(--border-main)] bg-[var(--bg-card)] sticky top-0 z-50 transition-colors duration-300">
                <div className="flex items-center gap-1 hover:scale-105 transition-transform duration-300 cursor-default group">
                    <span className="text-2xl sm:text-3xl font-black italic tracking-tighter uppercase text-black bg-[var(--primary)] px-2 py-0.5 rounded-sm shadow-[4px_4px_0px_0px_var(--text-main)] transition-all duration-300 group-hover:shadow-[6px_6px_0px_0px_var(--secondary)]">Edu</span>
                    <span className="text-2xl sm:text-3xl font-black tracking-tighter uppercase text-[var(--text-main)] transition-colors duration-300 group-hover:text-[var(--secondary)]">Pod.</span>
                </div>
                <div className="flex gap-4">
                    {!loading && (
                        <Link href={user ? "/home" : "/login"} className="touch-target btn-neo text-sm sm:text-base">
            ...
            <main className="flex-1">
                <section className="responsive-container py-12 sm:py-20 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 items-center">
                    <div className="space-y-6 sm:space-y-8">
                        ...
                        <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black uppercase tracking-tighter leading-[0.9] text-[var(--text-main)] transition-colors duration-300 drop-shadow-lg">
                            Turn Text<br />
                        ...
                        <p className="text-base sm:text-lg md:text-xl font-medium text-[var(--text-muted)] max-w-md border-l-4 border-[var(--primary)] pl-4 sm:pl-6 transition-colors duration-300">
                            Transform boring PDFs into immersive, 3D classroom experiences with AI-powered teachers.
                        </p>
                        <div className="flex flex-wrap gap-3 sm:gap-4">
                            {!loading && (
                                <Link href={user ? "/home" : "/login"} className="touch-target px-5 sm:px-8 py-3 sm:py-4 bg-[var(--text-main)] text-[var(--bg-card)] font-black uppercase tracking-widest text-sm sm:text-lg border-2 border-[var(--border-main)] hover:opacity-90 transition-all flex items-center gap-2 group shadow-[var(--shadow-block)] hover:shadow-[var(--shadow-block-hover)] hover:scale-105">
                                    {user ? "Dashboard Open" : "Get Started"} <ArrowRight className="group-hover:translate-x-1 transition-transform" />
                                </Link>
                            )}
                            <button
                                onClick={() => setShowSample(true)}
                                className="touch-target px-5 sm:px-8 py-3 sm:py-4 bg-[var(--bg-card)] text-[var(--text-main)] font-black uppercase tracking-widest text-sm sm:text-lg border-2 border-[var(--border-main)] hover:bg-[var(--bg-main)] transition-all shadow-[var(--shadow-block)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[var(--shadow-block-hover)] flex items-center gap-2 group">
                ...
                {/* Sample Video Modal */}
                {showSample && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-sm md:p-8">
                        <div className="mobile-modal-shell bg-[var(--bg-main)] border-4 border-[var(--border-main)] w-full max-w-6xl relative shadow-[16px_16px_0px_0px_var(--primary)] flex flex-col max-h-[100dvh] sm:max-h-[90vh] sm:h-auto md:h-[80vh] animate-in fade-in zoom-in duration-300 rounded-none sm:rounded-xl overflow-hidden">
                            {/* Window Header */}
                ...
                <section className="bg-[var(--bg-card)] border-t-2 border-[var(--border-main)] py-20 transition-colors duration-300">
                    <div className="responsive-container max-w-6xl mx-auto">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="p-6 sm:p-8 border-2 border-[var(--border-main)] hover:shadow-[8px_8px_0px_0px_var(--secondary)] transition-all bg-[var(--bg-main)] hover:-translate-y-2 card-swiss">
                                <BookOpen className="w-12 h-12 mb-6 text-[var(--secondary)]" />
                                <h3 className="text-xl sm:text-2xl font-black uppercase mb-2 text-[var(--text-main)]">PDF to Audio</h3>
                            ...
                            <div className="p-6 sm:p-8 border-2 border-[var(--border-main)] hover:shadow-[8px_8px_0px_0px_var(--primary)] transition-all bg-[var(--bg-main)] hover:-translate-y-2 card-swiss">
                                <Layers className="w-12 h-12 mb-6 text-[var(--text-main)]" />
                                <h3 className="text-xl sm:text-2xl font-black uppercase mb-2 text-[var(--text-main)]">3D Classroom</h3>
                            ...
                            <div className="p-6 sm:p-8 border-2 border-[var(--border-main)] hover:shadow-[8px_8px_0px_0px_var(--accent)] transition-all bg-[var(--bg-main)] hover:-translate-y-2 card-swiss">
                                <Zap className="w-12 h-12 mb-6 text-[var(--accent)]" />
                                <h3 className="text-xl sm:text-2xl font-black uppercase mb-2 text-[var(--text-main)]">Interactive Quiz</h3>
            ...
            <footer className="bg-black text-white border-t-2 border-[var(--border-main)] py-12 text-center transition-colors duration-300 dark:border-white">
                <p className="font-black uppercase tracking-widest text-sm text-[var(--primary)]">(C) 2025 EduPod V2. Built for Builders.</p>
            </footer>
        </div>
    );
}
