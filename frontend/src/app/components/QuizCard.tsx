...
const QuizCard: React.FC<QuizCardProps> = ({ questions, onComplete }) => {
    ...
    if (completed) {
        ...
        return (
            <motion.div
                ...
                className="w-full max-w-2xl mx-auto bg-[var(--bg-card)] border-4 border-[var(--border-main)] shadow-[12px_12px_0px_0px_var(--primary)] p-6 sm:p-12 text-center"
            >
                <Trophy className={`w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-6 ${percentage >= 70 ? 'text-[var(--primary)]' : 'text-[var(--text-muted)]'}`} />
                <h2 className="text-3xl sm:text-5xl font-black uppercase tracking-tighter mb-4 text-[var(--text-main)]">Quiz Complete!</h2>
                <div className="inline-block bg-[var(--text-main)] text-[var(--bg-main)] px-4 sm:px-6 py-2 text-4xl sm:text-6xl font-black mb-8 transform -rotate-2">
                    {percentage}%
                </div>
                <p className="text-base sm:text-xl font-bold uppercase tracking-widest text-[var(--text-main)] mb-8">
    ...
    return (
        <div className="w-full max-w-3xl mx-auto">
            ...
            <div className="bg-[var(--bg-card)] border-4 border-[var(--border-main)] shadow-[12px_12px_0px_0px_var(--border-main)] p-4 sm:p-8 md:p-12 relative overflow-hidden">
                {/* Decorative Pattern */}
                ...
                {/* Question */}
                <h3 className="text-2xl sm:text-3xl font-black mb-6 sm:mb-10 leading-tight text-[var(--text-main)] relative z-10">
                    ...
                    {currentQuestion.options.map((option, idx) => {
                        ...
                        return (
                            <motion.button
                                ...
                                className={`w-full text-left p-4 sm:p-6 border-2 font-bold text-base sm:text-lg flex items-center justify-between group transition-all
                ...
                <AnimatePresence>
                    {showResult && (
                        <motion.div
                            ...
                            className="bg-[var(--bg-main)] border-2 border-[var(--border-main)] p-4 sm:p-6"
                        >
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm sm:text-base font-medium text-[var(--text-main)] italic">{currentQuestion.explanation}</p>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 flex justify-end">
                                <button
                                    onClick={handleNext}
                                    className="touch-target flex items-center gap-2 px-5 sm:px-8 py-3 bg-[var(--text-main)] text-[var(--bg-main)] font-black uppercase tracking-widest border-2 border-[var(--border-main)] hover:bg-[var(--secondary)] hover:text-black hover:border-black transition-colors"
                                >
