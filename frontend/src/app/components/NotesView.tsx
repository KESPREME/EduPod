...
const NotesView: React.FC<NotesViewProps> = ({ notes }) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const ratio = Math.min(pdfWidth / imgWidth, pdfHeight / imgHeight) * 1.8; // Scale up
    ...
    return (
        <div className="w-full max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-4xl font-black uppercase tracking-tighter text-[var(--text-main)] drop-shadow-[2px_2px_0px_var(--primary)]">Study Notes</h2>
                <button
            ...
            {/* SCREEN VIEW: Respects Theme (Dark/Light) */}
            ...
            {/* Error Message */}
            {pdfError && (
                <div className="mt-4 p-4 bg-red-100 border-2 border-red-500 text-red-700 font-bold text-center">
                    ⚠️ {pdfError}
                </div>
            )}

            {/* PRINT VIEW: Hidden Off-Screen, Always White "Paper" Style */}
            <div className="print-container-parent absolute top-0 left-[-9999px]">
                <div ref={notesRef} className="w-[800px] min-h-[1100px] bg-white text-black p-16 font-sans border border-gray-200">
                    ...
                    {/* PDF Content */}
                    <article className="prose prose-xl max-w-none prose-headings:font-black prose-headings:uppercase prose-headings:tracking-tight prose-headings:text-black prose-p:text-black prose-li:text-black prose-blockquote:border-l-8 prose-blockquote:bg-gray-50 prose-blockquote:p-6 prose-blockquote:not-italic prose-blockquote:text-gray-700" style={{ "--tw-prose-strong": "#5E17EB" } as React.CSSProperties}>
                        <ReactMarkdown>{notes}</ReactMarkdown>
                    </article>

                    {/* PDF Footer */}
                    <div className="mt-20 pt-6 border-t-2 border-gray-200 text-center text-xs font-bold uppercase text-gray-400">
                        Generated by EduPod AI • Learning Redefined
                    </div>
                </div>
            </div>
        </div>
